	...
	if (size == 0) {
		...
		for (int i=0; i<rank; i++) {
			for (int j=0; j<rank; j++) {
				//empty space are marked by 0 in the file
...
bool completeBoard(void*args){
	...
	for(int num=1;num<=rank;num++){
		if (duplicates(num,row,col)=true){
			board[row][col]=num;
			if (completeBoard()=true){
				return true;
			}
		}
		board[row][col]=0;
